%%%-------------------------------------------------------------------
%% @doc ttt (tic-tac-toe) Test Unit
%%
%% EUnit 's official doc: https://www.erlang.org/doc/apps/eunit/chapter.html
%%
%% @end
%%%-------------------------------------------------------------------
-module(ttt_test).
...
% %%% All Test Fixtures
start_test() ->
    {"It should be possible to start a ttt server and give it a name",
        {setup,
...
%%
%% Setups/teardowns
%% @TODO: read https://www.erlang.org/doc/apps/erts/time_correction.html
%% now() replaced by erlang:timestampe()
%%
%% @end
%% ----------------------------------
...
%% ----------------------------------
start_and_test_name(Name) ->
    ttt_sup:start(Name, 1, {gameserver_process, start, []}),
    A = whereis(Name),
